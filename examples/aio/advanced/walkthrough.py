...
async def _run_walkthrough(client):
    ...
    if table_info:
        ...
        table_info = await backoff(
            lambda: client.tables.create(table_name, columns, display_name="Walkthrough Demo")
        )
    ...
    batch.records.create(
        table_name,
        {"new_Title": "Batch task alpha", "new_Quantity": 1, "new_Amount": 25.0, "new_Completed": False, "new_Priority": Priority.LOW},
    )
    batch.records.create(
        table_name,
        {"new_Title": "Batch task beta", "new_Quantity": 2, "new_Amount": 50.0, "new_Completed": False, "new_Priority": Priority.MEDIUM},
    )
    ...
    async with batch.changeset() as cs:
        cs_ref = cs.records.create(
            table_name,
            {"new_Title": "Changeset task", "new_Quantity": 5, "new_Amount": 100.0, "new_Completed": False, "new_Priority": Priority.HIGH},
        )
