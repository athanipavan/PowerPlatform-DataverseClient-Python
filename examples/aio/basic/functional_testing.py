        ...
        for attempt in range(1, retries + 1):
            try:
                ...
                if getattr(err, "status_code", None) == 404 and attempt < retries:
                    print(f"   Table not ready for create (attempt {attempt}/{retries}). Retrying in {delay_seconds}s...")
                    await asyncio.sleep(delay_seconds)
                    continue
                raise
...
async def test_batch_all_operations(client: AsyncDataverseClient, table_info: Dict[str, Any]) -> None:
    ...
    try:
        ...
        batch.records.create(
            table_schema_name,
            {f"{attr_prefix}_name": f"Batch-A {datetime.now().strftime('%H:%M:%S')}", f"{attr_prefix}_count": 1, f"{attr_prefix}_is_active": True},
        )
        batch.records.create(
            table_schema_name,
            [
                {f"{attr_prefix}_name": f"Batch-B {datetime.now().strftime('%H:%M:%S')}", f"{attr_prefix}_count": 2, f"{attr_prefix}_is_active": True},
                {f"{attr_prefix}_name": f"Batch-C {datetime.now().strftime('%H:%M:%S')}", f"{attr_prefix}_count": 3, f"{attr_prefix}_is_active": True},
            ],
        )
        ...
        if all_ids:
            ...
            async with batch.changeset() as cs:
                ref = cs.records.create(
                    table_schema_name,
                    {f"{attr_prefix}_name": f"Batch-D {datetime.now().strftime('%H:%M:%S')}", f"{attr_prefix}_count": 4, f"{attr_prefix}_is_active": False},
                )
        ...
        async with batch.changeset() as cs:
            cs.records.create(
                table_schema_name,
                {f"{attr_prefix}_name": f"Rollback-test {datetime.now().strftime('%H:%M:%S')}", f"{attr_prefix}_count": 0, f"{attr_prefix}_is_active": False},
            )
